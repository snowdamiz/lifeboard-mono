...
- [ ] **CODE-01**: Replace all `ilike/2` calls with `like/2` across all Ecto query files (35+ occurrences in `search.ex`, `inventory.ex`, `receipts.ex`, `goals.ex`, `templates.ex`)
- [ ] **CODE-02**: Set `async: false` on all ExUnit test cases that use database connections (SQLite's single-writer model is incompatible with async sandbox)
