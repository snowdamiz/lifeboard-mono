...
if purchases_sheet && pantry_sheet do
  ...
  IO.puts(
    "ERROR: Missing sheets. Purchases: #{inspect(purchases_sheet)}, Pantry: #{inspect(pantry_sheet)}"
  )
...
target_item =
  Repo.one(from i in Item, where: i.sheet_id == ^pantry_sheet.id and i.name == ^item_name)
