defmodule MegaPlannerWeb.GoalCategoryController do
  ...
  def update(conn, %{"id" => id, "category" => category_params}) do
    ...
    with category when not is_nil(category) <-
           Goals.get_household_goal_category(user.household_id, id),
         {:ok, %GoalCategory{} = category} <-
           Goals.update_goal_category(category, category_params) do
      json(conn, %{data: category_to_json(category)})
    else
      nil -> {:error, :not_found}
      error -> error
  ...
  defp category_to_json(category) do
    %{
      ...
      parent:
        if(Ecto.assoc_loaded?(category.parent) && category.parent,
          do: %{
      ...
      subcategories:
        if(Ecto.assoc_loaded?(category.subcategories),
          do:
            Enum.map(category.subcategories, fn sub ->
              %{
                id: sub.id,
                name: sub.name,
                color: sub.color,
