defmodule MegaPlannerWeb.BudgetController do
  ...
  def summary(conn, params) do
    ...
    regular_entries =
      Budget.list_entries(user.household_id,
        start_date: start_date,
        end_date: end_date,
        exclude_purchases: true
      )

    {reg_income, reg_expense} =
      Enum.reduce(regular_entries, {Decimal.new(0), Decimal.new(0)}, fn entry, {inc, exp} ->
    ...
    trips =
      Receipts.list_trips(user.household_id,
        start_date: start_date,
        end_date: end_date
      )

    trips_with_purchases =
      Enum.filter(trips, fn trip ->
        trip.stops &&
          Enum.any?(trip.stops, fn stop -> stop.purchases && length(stop.purchases) > 0 end)
      end)

    trip_expense =
      Enum.reduce(trips_with_purchases, Decimal.new(0), fn trip, acc ->
        trip_total =
          Enum.reduce(trip.stops || [], Decimal.new(0), fn stop, trip_acc ->
            stop_total =
              Enum.reduce(stop.purchases || [], Decimal.new(0), fn purchase, stop_acc ->
                tax =
                  Receipts.calculate_tax(stop.store_id, purchase.total_price, purchase.taxable)
