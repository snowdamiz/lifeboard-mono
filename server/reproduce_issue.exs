...
{:ok, entry} =
  MegaPlanner.Budget.create_entry(%{
...
{:ok, purchase} =
  Receipts.Purchase.changeset(%Receipts.Purchase{}, %{
    "item" => "New Purchase Item",
    "store_code" => store_code,
    "total_price" => Decimal.new("10.00"),
    # Does purchase have quantity? No, it has units/count/etc.
    "quantity" => 1,
    "count" => 1,
    "household_id" => household_id,
...
purchases_items =
  Repo.all(
    from i in Inventory.Item,
      where: i.sheet_id == ^purchases_sheet.id and i.store_code == ^store_code
  )
