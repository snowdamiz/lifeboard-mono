Application.put_env(:mega_planner, MegaPlannerWeb.Endpoint,
  http: [port: 0],
  server: false,
  url: [host: "localhost"]
)
...
week =
  Repo.all(
    from t in "tasks",
      where: t.date >= ^~D[2026-02-02] and t.date <= ^~D[2026-02-08],
      select: %{
        id: t.id,
        title: t.title,
        date: t.date,
...
File.write!(
  "db_check_results.txt",
  File.read!("db_check_results.txt") <>
    "=== TASKS THIS WEEK (Feb 2-8) ===\nFound: #{length(week)}\n#{week_output}\n\n"
)
...
trips =
  Repo.all(
    from t in "trips",
      select: %{id: t.id, trip_start: t.trip_start, household_id: t.household_id},
      order_by: [desc: t.inserted_at],
      limit: 10
  )
