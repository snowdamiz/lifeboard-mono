...
Enum.each(items, fn i ->
  IO.puts(
    "ID: #{i.id} | Name: '#{i.name}' | Brand: '#{i.brand}' | Qty: #{i.quantity} | Store: '#{i.store}'"
  )
end)
...
purchases =
  Repo.all(
    from p in Purchase,
      ...
      select: [:id, :item, :brand, :units, :inserted_at]
  )

Enum.each(purchases, fn p ->
  IO.puts(
    "ID: #{p.id} | Item: '#{p.item}' | Brand: '#{p.brand}' | Units: #{p.units} | Date: #{p.inserted_at}"
  )
end)
