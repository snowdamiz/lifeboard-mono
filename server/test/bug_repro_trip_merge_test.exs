defmodule MegaPlanner.BugReproTripMergeTest do
  ...
  test "purchase of existing item should NOT merge into generic inventory automatically" do
    ...
    {:ok, pantry} =
      Inventory.create_sheet(%{household_id: household_id, name: "Pantry", user_id: user.id})

    {:ok, purchases_sheet} =
      Inventory.create_sheet(%{household_id: household_id, name: "Purchases", user_id: user.id})
    ...
    {:ok, trip} =
      Receipts.create_trip(%{trip_start: DateTime.utc_now(), household_id: household_id})
    ...
    assert updated_pantry.quantity == 2,
           "Bug not reproduced: Pantry quantity should have increased"

    refute purchase_sheet_item,
           "Bug not reproduced: Purchase item should NOT exist in staging yet"
  end
end
