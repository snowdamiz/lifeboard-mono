...
purchases =
  from(p in Purchase, where: ilike(p.brand, "Brand3"), order_by: [desc: p.inserted_at])
  |> Repo.all()
...
if Enum.empty?(purchases) do
  ...
  Enum.each(purchases, fn p ->
    IO.puts(
      "ID: #{p.id}, Brand: #{p.brand}, StoreCode: #{inspect(p.store_code)}, ItemName: #{inspect(p.item_name)}"
    )
  end)
end
