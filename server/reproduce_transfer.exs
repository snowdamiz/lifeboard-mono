...
defmodule Helper do
  ...
  def setup_test_data(household_id, user_id) do
    ...
    {:ok, source_sheet} =
      case Repo.get_by(Inventory.Sheet, household_id: household_id, name: "Purchases") do
        nil ->
    ...
    {:ok, target_sheet} =
      case Repo.get_by(Inventory.Sheet, household_id: household_id, name: "Pantry") do
        nil ->
          Inventory.create_sheet(%{
            "name" => "Pantry",
            "household_id" => household_id,
    ...
    case Inventory.create_item(%{
      ...
      {:ok, item} ->
        ...
        case Inventory.create_shopping_list(%{
               "name" => "Debug List #{System.unique_integer([:positive])}",
               "household_id" => household_id,
               "user_id" => user_id
             }) do
          {:ok, list} ->
...
if household_id && user_id do
  ...
  case Helper.setup_test_data(household_id, user_id) do
    {:ok, item, source_sheet, target_sheet} ->
      FileLogger.log(
        "Created Source Item: #{item.name} (ID: #{item.id}) in '#{source_sheet.name}'"
      )
      ...
      if updated_item do
        FileLogger.log(
          "Source Item Quantity after Partial: #{updated_item.quantity} (Expected: 3)"
        )
      else
