...
export const useNotesStore = defineStore('notes', () => {
  ...
  async function fetchNotebooks() {
    return fetchIfStale('notes:notebooks', async () => {
      loading.value = true
      try {
  ...
  async function updateNotebook(id: string, updates: Partial<Notebook> & { tag_ids?: string[] }) {
    ...
    if (index !== -1) notebooks.value[index] = response.data
    if (currentNotebook.value?.id === id) currentNotebook.value = response.data
    invalidate('notes:notebooks')
    return response.data
  }
  ...
  async function updatePage(id: string, updates: Partial<Page> & { tag_ids?: string[] }) {
    ...
    if (currentPage.value?.id === id) currentPage.value = response.data
    return response.data
  }
  ...
  return {
    notebooks, currentNotebook, pages, currentPage, loading,
    notebookFilterTags, pageFilterTags,
    fetchNotebooks, fetchNotebook, createNotebook, updateNotebook, deleteNotebook,
