...
interface Props {
  tripId: string
  inlineMode?: boolean  // Adjusts styling for inline vs modal context
}
...
// Styling classes that adjust based on context
const headerBorderClass = computed(() => props.inlineMode ? 'border-white/[0.08]' : 'border-border')
const footerBorderClass = computed(() => props.inlineMode ? 'border-white/[0.08]' : 'border-border')
...
const handleReceiptScanned = async () => {
  ...
  await loadTrip()
  // Also refresh calendar trips so TripCard shows updated purchases/totals
  await calendarStore.fetchCurrentViewTasks()
}

onMounted(() => {
  console.log('[DEBUG] TripDetailContent Mounted')
  loadTrip()
})
</script>
...
<template>
  <div class="h-full flex flex-col">
    <!-- Header -->
    <div :class="['flex items-center justify-between p-4 border-b shrink-0 bg-secondary/30', headerBorderClass]">
      <div class="flex-1">
        <h2 class="text-lg font-semibold">Manage Trip</h2>
        <div class="flex items-center gap-3 mt-2">
           <DateChooser 
              :model-value="getPurchaseDate()"
              class="text-sm"
              @update:model-value="(v) => updateTripDate(v)"
            />
    ...
    <!-- Footer -->
    <div :class="['flex gap-3 p-4 border-t bg-card shrink-0', footerBorderClass]">
